...
# -*- coding: utf-8 -*-
"""
This experiment was created using PsychoPy2 Experiment Builder (v1.81.03), February 24, 2015, at 13:41
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Josh\\Dropbox\\bin\\EXPLORATION\\jkpsycho_install\\test.psyexp',
...
# Initialize components for Routine "looper_instr"
looper_instrClock = core.Clock()
pause_text_4 = visual.TextStim(win=win, ori=0, name='pause_text_4',
    text='default text',    font='Arial',
    pos=[0, 0], height=0.1, wrapWidth=None,
    color='white', colorSpace='rgb', opacity=1,
    depth=0.0)
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    ######################################################################
    # timing logic
    if t >= 4 and tr1.status == NOT_STARTED:
        # keep track of start time/frame for later
        tr1.tStart = t  # underestimates by a little under one frame
        tr1.frameNStart = frameN  # exact frame index
        tr1.status=STARTED
    if tr1.status == STARTED and t >= (4 + (6-win.monitorFramePeriod*0.75)): #most of one frame period left
        tr1.status = STOPPED
    ######################################################################
...
#------Prepare to start Routine "looper_instr"-------
t = 0
looper_instrClock.reset()  # clock 
frameN = -1
# update component parameters for each repeat
pause_text_4.setText('The next test is for TableLooper.  Press 5 to step through the table, 6 to move on to the next test when you are done.  The row will be printed on screen for you.')
event.clearEvents(eventType='keyboard')

_text = 'The next test is for TableLooper.  Press 5 to step through the table, 6 to move on to the next test when you are done.  The row will be printed on screen for you.'+'\n\n'
if None:
    _text+='press a key to continue:\n'+str(list(None))[1:-1]
else:
    _text+='press any key to continue'
pause_text_4.setText(_text)

# keep track of which components have finished
looper_instrComponents = []
looper_instrComponents.append(pause_text_4)
for thisComponent in looper_instrComponents:
    if hasattr(thisComponent, 'status'):
        thisComponent.status = NOT_STARTED

#-------Start Routine "looper_instr"-------
continueRoutine = True
while continueRoutine:
    # get current time
    t = looper_instrClock.getTime()
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
    
    # *pause_text_4* updates
    if t >= 0.0 and pause_text_4.status == NOT_STARTED:
        # keep track of start time/frame for later
        pause_text_4.tStart = t  # underestimates by a little under one frame
        pause_text_4.frameNStart = frameN  # exact frame index
        pause_text_4.setAutoDraw(True)
    # stop routine if pause_text_4's buttons are pressed
    if pause_text_4.status==STARTED:
        if event.getKeys(keyList=None): stopRoutine()
...
trials = data.TrialHandler(nReps=500, method='random', 
    extraInfo=expInfo, originPath='C:\\Users\\Josh\\Dropbox\\bin\\EXPLORATION\\jkpsycho_install\\test.psyexp',
    trialList=[None],
...
for thisTrial in trials:
    ...
    # works for now... but rather unintuitive.
